...
const Chart = (props) => {
    ...
    Object.keys(timeVALUE).map((each) => {
        ...
        testdata.push({
            name: `${timeVALUE[each]}`,
            value: parseFloat(chartStockvalue[each]).toFixed(2),
            pv: 3000,
            amt: 7000,
        });
    ...
    const renderLineChart = (
        <LineChart
            ...
            height={Chartheight}
            data={testdata}
            margin={{ top: 50, right: 0, bottom: 20, left: Chartmarginleft }}
        >
            <Line
                type="monotone"
                dataKey="value"
    ...
    return (
        <div className="chart">
            <h2>{title}</h2>
            <p className="cost">
                <span>{price.value || "NA"}</span> {price.currency}
            </p>
            ...
            <div className="time-filter">
                <span
                    className={"filter " + (filter == "3" ? "active" : "")}
                    name="3"
                    onClick={(event) =>
                        setfilter(event.target.getAttribute("name"))
                    }
                >
                    3 months
                </span>
                <span
                    className={"filter " + (filter == "6" ? " active" : "")}
                    name="6"
                    onClick={(event) =>
                        setfilter(event.target.getAttribute("name"))
                    }
                ...
                <span
                    className={"filter " + (filter == "12" ? " active" : "")}
                    name="12"
                    onClick={(event) =>
                        setfilter(event.target.getAttribute("name"))
                    }
